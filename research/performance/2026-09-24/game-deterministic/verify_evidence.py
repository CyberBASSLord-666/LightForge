...
with deterministic compute requested for the heavy CUDA sessions.
...
SOURCE = '9ca73bb8681434f6f2dc9ce1a00ce33531a04379'
SOURCE_TREE = 'a5a7071ce9252608bcb7b5d57126396669bd7abe'
...
HARNESS_SHA = 'd67fa0fe0d75bba041a1b9cf00a55a58d65fc81c93f25b9d76247d62c4bab527'
...
def verify_snapshots(evidence, receipt, harness, kind):
    ...
    for variant in receipt['variants']:
        for mode in receipt['modes']:
            ...
            changed = harness.variant_source(source, variant, cuda_heavy_only=True, cuda_deterministic=True)
            if mode == 'profiled':
...
def verify_handoff(root, receipt, repo):
    ...
    expected = dict(schema='lightforge.game-deterministic-handoff.v1', executionSourceCommit=SOURCE,
                    ...
                    harnessSha256=HARNESS_SHA, cudaHeavyOnly=True, cudaDeterministicCompute=True)
    for key, value in expected.items():
        require(handoff.get(key) == value, 'Execution/derivation handoff mismatch: ' + key)
    require(handoff.get('cudaHeavyOnly') is True and handoff.get('cudaDeterministicCompute') is True,
            'Both deterministic handoff flags must be boolean true')
...
def verify(args, extracted):
    ...
    require(receipt.get('cudaHeavyOnly') is True and receipt.get('cudaDeterministicCompute') is True,
            'Explicit heavy-only and deterministic CUDA requests are required')
    ...
    for stage in ('readiness', 'qualification'):
        exit_record = strict_json(root / (stage + '-exit.json'))
        stage_receipt_path = root / stage / 'receipt.json'
        require(type(exit_record.get('exitCode')) is int and exit_record['exitCode'] == 0,
                'Nonzero/missing process exit: ' + stage)
        require(exit_record['logSha256'] == sha(root / (stage + '.log'))
                and exit_record['receiptSha256'] == sha(stage_receipt_path),
                'Closed process log/receipt hash mismatch: ' + stage)
        if stage == 'readiness':
            readiness = strict_json(stage_receipt_path)
            require(readiness['status'] == 'PREFLIGHT_READY'
                    and all(readiness.get(flag) is False for flag in FLAGS), 'Readiness did not pass without approval')
            for key in ('sourceHashes', 'dependencyHashes', 'inputProvenance', 'inputProvenanceSha256',
                        'modelManifestSha256', 'modelHashes', 'variants', 'modes', 'cudaOptions',
                        'cudaHeavyOnly', 'cudaDeterministicCompute', 'requestedGraphProviders'):
                require(readiness.get(key) == receipt.get(key), 'Readiness/qualification binding mismatch: ' + key)
    ...
    return dict(schema='lightforge.game-evidence-verification.v1', verificationPassed=True,
                ...
                sourceHandoff=handoff_report, cudaHeavyOnly=True, cudaDeterministicCompute=True,
