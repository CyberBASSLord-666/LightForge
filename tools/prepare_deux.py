...
Install tools/model-requirements.txt and CPU torch==2.13.0 first.
"""
import sys,pathlib,time,types,json,hashlib,argparse,urllib.request,tempfile,os
from model_export_guard import verify_digest, exporter_identity, cached_graph_matches
...
verify_digest(checkpoint,SHA,'Deux checkpoint')
identity=exporter_identity(ROOT)
...
for name,m,example in modules:
 ...
 cached=cached_graph_matches(dest,previous,converter_sha,identity)
 if not cached:
  ...
  with torch.inference_mode():torch.onnx.export(m.eval(),example,str(dest),input_names=['input'],output_names=['output'],dynamic_axes=dynamic,opset_version=17,do_constant_folding=True,dynamo=False)
