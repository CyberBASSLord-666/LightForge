...
The current published release is [LightForge 2.2.2](https://github.com/CyberBASSLord-666/LightForge/releases/tag/v2.2.2), Android version code 20202. Its diagnostic logging, Downloads export and renderer-recovery update passes all seven source-bound release gates; the complete APK retains the original signing identity. The complete [production verification run, attempt 2](https://github.com/CyberBASSLord-666/LightForge/actions/runs/34179649874/attempts/2) passed the current pipeline, Android lifecycle and diagnostic export checks, with numerical kernel evidence retained under its explicit immutable-source protocol. The [publication workflow](https://github.com/CyberBASSLord-666/LightForge/actions/runs/34183955111) passed and verified the uploaded APK digest against the signed update. Use [BUILD.md](BUILD.md) for the current model/build pipeline and [VALIDATION.md](VALIDATION.md) for evidence.

Current sources, model converters, manifests, tests, workflow definitions and maintenance documents are integrated at the repository root. Version 2.2.2 adds bounded local diagnostic reporting and guarded renderer cleanup while retaining native CPU Studio separation and durable passage/stage recovery. Current verification belongs in `qa/release-2.2.2/`; the 2.2.1 evidence remains in its historical directory. Native dependencies are pinned in `android/native-runtime.json`. Historical source and QA folders retain their original version identities. `verify_snapshot.py` checks the historical preserved snapshot; changes in current application files are expected and must instead pass the current release gates.
