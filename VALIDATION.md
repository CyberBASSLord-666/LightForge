...
The 2.2.4 candidate preserves the pinned models, denoise/transcription settings and sample clock. The original-signed build and component/browser checks pass; the latest focused Android diagnostics pass eight checks on a temporary APK. The corrected-fixture Android run still fails project reconnection at the 45-second readiness deadline. The portable settings proof demonstrates and repairs one fixture mismatch in hosted app/worker execution; it does not establish Android completion. Release remains blocked on the full lifecycle and final seven-gate packaging.
...
| Chromium UI and full pipeline | Current verification/build job passed responsive UI/worker/WebGL, actual Precision model analysis, saved voice audio, cancellation and FSEQ restore. This is separate from the failed corrected Android lifecycle run. |
...
| Android runtime lifecycle | **Failed after fixture correction.** Run 34424613238 saved screen-off Studio work in 311.944 seconds, then exceeded the 45-second reopened-preview readiness deadline. Its last retained JavaScript sample was taken 21.23 seconds into the wait, not at the deadline; it showed composition pending, model not ready and zero preview frames. Reconnection, Balanced, Cancel/Resume and timeout cleanup remain unverified. |
| Fixture settings component proof | The [portable actual-app/worker reproduction](qa/release-2.2.4/fixture-settings/README.md) passes both expected cases: sparse settings correctly fail restore; complete actual UI settings preserve input/frame hashes, clear pending state and avoid repeated restore. Sources are hashed in its receipt. This does not run audio models, the GPU preview or Android. |
| Android diagnostics | **Passed, 8 checks on the focused temporary APK.** Run 34424613238 covers crash/tombstone persistence, compatibility selection, readable Downloads reports, selected-URI export, actual Guide JavaScript export and detached/repeated renderer recovery on API 35. This does not certify background lifecycle completion or make the temporary APK release eligible. |
...
Original evidence remains separate for the [first teardown ANR](qa/release-2.2.4/android-first-run/README.md), [preview-readiness timeout](qa/release-2.2.4/android-preview-first-frame-run/README.md), [earlier focused diagnostic failure](qa/release-2.2.4/android-readiness-diagnostic-run/README.md), and [startup follow-up with failed background/passed diagnostics](qa/release-2.2.4/android-startup-followup-run/README.md). The [corrected-fixture run](qa/release-2.2.4/android-corrected-fixture-run/README.md) is also preserved with its failed background and eight-check diagnostics pass. The demonstrated fixture bug does not turn any failed record into a passing gate and does not fully explain the remaining failure after correction. The retained 2.2.2 and 2.2.1 sections below describe their original evidence and are not promoted to 2.2.4 results.
