...
Version 2.2.2 / code 20202 adds local diagnostic collection, Downloads export and guarded renderer recovery. Current evidence belongs in `qa/release-2.2.2/`. This section starts with every current gate pending; no historical pass is promoted to a current result.

| Current check | Status and scope |
| --- | --- |
| Version synchronization | Version, package metadata and generated web version are synchronized to 2.2.2 / 20202. |
| Node/Python regressions | Pending against the final 2.2.2 source. |
| Native compilation and diagnostic tests | Pending. Verify bounded storage, redaction, report structure, recovery after partial writes, and export failures without losing stored logs. |
| Browser diagnostics and responsive UI | Pending. Verify Guide and error-state export actions, captured errors and rejected promises, report success/failure feedback, and no layout overflow. |
| Android renderer and Downloads behavior | Pending. Verify detached/retired renderer cleanup, safe foreground recovery, real Downloads visibility, and available process-exit metadata. Desktop mocks do not establish device behavior. |
| Native Studio lifecycle | Pending against the final source. Current Android execution remains a separate gate from historical 2.2.1 screen-off/Resume evidence. |
| Analysis numerical evidence | **Retained with exact measured-source verification.** Every source/model bound by the 2.2.1 numerical evidence is byte-identical except the explicitly archived and validated release name/code metadata. This is not a fresh benchmark. Nine retention-validator tests reject model/code changes, invalid or unauthenticated metadata, failed/wrong-release evidence, traversal, and preserve existing receipts on failure. |
| Signed update and publication | Pending. Require current-source receipts, full asset matching, native ABIs, original signing identity, version and uploaded checksum. |
| Physical phone and Tesla | Unverified. No claim of resolution for the user's specific device crash without its report and a confirming device run. |

Reports are collected locally and exported only at the user's request. Bounded logs and Android process-exit information improve diagnosis but cannot guarantee a final stack after every native crash, low-memory kill or abrupt process termination. Report tests must cover retention limits, common-secret redaction, unavailable system metadata, and failed export cleanup.
