...
The 2.2.4 candidate preserves the pinned models, denoise/transcription settings and sample clock. Three Android attempts remain failed or incomplete. The latest startup fix gives completed-project restoration a single owner, preserves pending navigation, and defers preview rendering/environment generation until the vehicle is loaded and visible. All 347 Node and 69 Python tests pass locally with zero skips, including 22 focused startup checks; the tested completed Studio/Night renders are pixel-identical. This does not establish Android lifecycle completion: a fresh complete run and newly built original-signed APK are pending.

| 2.2.4 check | Current result and scope |
| --- | --- |
| Assets and host regression | Current local full regression passed all 347 Node and 69 Python tests with zero skips; 73 analysis assets matched. The source-bound local receipt completed on 2026-09-10 at 00:44:24 UTC. Fresh full CI and Android execution remain pending. |
...
| Android compilation and APK | The preceding APK passed its build/signature checks, but its contents predate the latest startup fix. A new original-signed build and all final source/package checks are required; the earlier APK is stale for this candidate. |
| Chromium UI and full pipeline | The preceding source passed responsive UI/worker/WebGL, actual Precision model analysis, saved voice audio, cancellation and FSEQ restore. The startup fix has the focused and pixel comparisons above; fresh complete browser verification remains required. |
...
| Android runtime lifecycle | **Failed/incomplete.** Focused diagnostic run 34420755126 saved screen-off Studio work, then exceeded the 45-second readiness deadline before the visual-state callback. At 32.98 seconds composition/background restoration were pending, the model was not ready, and the preview was visible/unpaused after two frames. Reconnection, Balanced, Cancel/Resume and timeout cleanup remain unverified for the new candidate. |
| Android diagnostics | **Failed/incomplete.** The focused log records six native-side checks before JavaScript export timed out. No LightForge ANR/OOM was identified; boot-time Google Play services ANRs do not prove a contemporaneous cause. The diagnostic APK has a temporary signer and is not release eligible. |
...
Original failed evidence is retained separately for the [first teardown ANR](qa/release-2.2.4/android-first-run/README.md), [follow-up preview-readiness timeout](qa/release-2.2.4/android-preview-first-frame-run/README.md) and [focused readiness/diagnostics run](qa/release-2.2.4/android-readiness-diagnostic-run/README.md). None is promoted to a passing gate. The retained 2.2.2 and 2.2.1 sections below describe their original evidence and are not promoted to 2.2.4 results.
