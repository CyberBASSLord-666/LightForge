...
The native/WASM same-input comparison and reproduction path from 2.2.1 remain in [qa/release-2.2.1/DEUX_RUNTIME.md](qa/release-2.2.1/DEUX_RUNTIME.md). When all measured sources bound by its numerical evidence remain byte-identical, `python3 tools/verify_retained_analysis.py --from-release 2.2.1 --prior-version-file qa/release-2.2.2/prior-version-2.2.1.json` may create an explicitly retained numerical receipt. It must fail if any measured source changes. Otherwise reproduce affected comparisons and role/reference checks before aggregating with `python3 qa/release-2.2.2/verify-analysis.py`. This aggregator checks existing exact evidence; it does not perform inference or repair stale hashes. Fresh complete-browser and Android execution are required separately.
...
For a release whose measured analysis implementation is unchanged, `python3 tools/verify_retained_analysis.py --from-release 2.2.1 --prior-version-file qa/release-2.2.2/prior-version-2.2.1.json` verifies every measured-source hash before retaining the earlier numeric evidence. The explicit prior version file was fetched from the immutable 2.2.1 candidate commit and must match its original evidence hash; a strictly validated name/code migration is recorded separately. Changed native transforms, graph partitioning, source-clock handling or other measured sources require new evidence. The public-browser inference and Android native Studio gates run independently, including diagnostics-enabled code.
