...
Prepare the licensed MUSDB reference excerpts as shown in `.github/workflows/verify-v2.yml` before running the actual model browser test. The packager requires six passing, source-bound receipts: regression, browser, native, complete browser analysis, numerical/reference analysis and Android background lifecycle. The emulator receipt cannot be generated by running the desktop commands above. Current 2.2.2 harnesses live in `qa/release-2.2.2/`; old passing receipts are not copied into that directory.
