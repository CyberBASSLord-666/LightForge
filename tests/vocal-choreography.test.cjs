...
test('bridge consumes only a freshly recomputed non-linguistic acoustic link and applies bounded hints',()=>{
  ...
  const protectedPriority=strategy.classifyCandidate({...original,priority:91});
  assert.equal(protectedPriority.priority,91,'the bridge must never lower an existing structural/climax priority');
...
test('canonical v2 validation makes schema, container, and link corruption inert with exact legacy output',()=>{
  const settings={stepMs:20,dance:'balanced',seed:113,vocalFocus:1,vocalChoreography:true};
  const baseline=Engine.generate(music(false),{...settings,vocalChoreography:false});
  const variants=[
    ['schema-version',value=>{value.semanticTimeline.schemaVersion=3;}],
    ['event-string',value=>{value.semanticTimeline.events='not-an-event-list';}],
    ['null-event',value=>{value.semanticTimeline.events=[null];}],
    ['malformed-link',value=>{value.vocalSemanticLinks.phrases='not-a-link-list';}]
  ];
  const prior=globalThis.LightForgeVocalSemantics;
  globalThis.LightForgeVocalSemantics=semanticsApi();
  try{
    for(const [label,mutate] of variants){
      const invalid=music();mutate(invalid);
      const result=Engine.generate(invalid,settings);
      assert.equal(result.choreography.vocalChoreography.active,false,label);
      assert.deepEqual(result.frames,baseline.frames,label+' must retain legacy frames');
      assert.deepEqual(Engine.fseq(result,'vocal.wav'),Engine.fseq(baseline,'vocal.wav'),label+' must retain legacy FSEQ');
    }
  }finally{
    if(prior===undefined)delete globalThis.LightForgeVocalSemantics;
    else globalThis.LightForgeVocalSemantics=prior;
  }
});
