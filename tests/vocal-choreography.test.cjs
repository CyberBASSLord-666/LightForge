...
test('browser and worker load acoustic semantics and bridge before ShowEngine',()=>{
  ...
  const semantics=html.indexOf('analysis/vocal-semantics.js'),bridge=html.indexOf('engine/vocal-choreography.js'),engine=html.indexOf('engine/show-engine.js');
  assert.ok(semantics>=0&&bridge>semantics&&engine>bridge);
  const worker=fs.readFileSync(path.join(__dirname,'../web/engine/worker.js'),'utf8');
  const workerSemantics=worker.indexOf("'../analysis/vocal-semantics.js'"),workerBridge=worker.indexOf("'vocal-choreography.js'"),workerEngine=worker.indexOf("'show-engine.js'");
  assert.ok(workerSemantics>=0&&workerBridge>workerSemantics&&workerEngine>workerBridge);
});
