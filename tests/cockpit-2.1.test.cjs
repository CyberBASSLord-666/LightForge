...
test('cockpit navigation preserves real transactional cue editing and project changes',async()=>{
 ...
 w.HTMLCanvasElement.prototype.getContext=()=>new Proxy({},{get:()=>()=>{}});w.scrollTo=()=>{};w.requestAnimationFrame=()=>0;w.cancelAnimationFrame=()=>{};w.matchMedia=()=>({matches:true,addEventListener(){}});
 ...
 try{
  w.eval(fs.readFileSync(path.join(root,'web/app.js'),'utf8'));w.eval(fs.readFileSync(path.join(root,'web/precision-studio.js'),'utf8'));w.eval(fs.readFileSync(path.join(root,'web/cockpit.js'),'utf8'));
  ...
  d.getElementById('workspace-review').click();assert.equal(d.getElementById('panel-review').hidden,false);assert.equal(d.getElementById('panel-compose').hidden,true);
