...
test('cached bass provenance migrates accompaniment mixtures without rereading stems',async()=>{
 ...
 const messages=[],writes=[],context=vm.createContext({console,URL,Float32Array,ArrayBuffer,DataView,performance,Map,Number,setTimeout,crypto:webcrypto,navigator:{hardwareConcurrency:2},importScripts(){},ort:{env:{wasm:{}}},LightForgeSemanticTimeline:require('../web/analysis/semantic-timeline.js'),LightForgeAnalysisStore:{open:async()=>({read:async()=>structuredClone(cached),write:async(name,value)=>writes.push([name,structuredClone(value)]),invalidate:async()=>{},reserve:async()=>{}})},fetch:async()=>({json:async()=>({precision:{},balanced:{},frontend:{}})}),postMessage:m=>messages.push(m)});
 ...
 const result=messages.at(-1);assert.equal(result.type,'result');assert.equal(result.restored,true);assert.equal(result.value.analysisVersion,8);assert.equal(result.value.bassAnalysis.source,'accompaniment-mixture-estimate');assert.equal(result.value.bassAnalysis.sourceSeparated,false);assert.equal(result.value.bassAnalysis.instrumentSeparated,false);assert.equal(result.value.bassNotes[0].sourceSeparated,false);assert.equal(result.value.roleAnalysis.version,5);assert.equal(result.value.roleAnalysis.bassInstrumentSeparated,false);assert.equal(result.value.semanticTimeline.schemaVersion,2);assert.equal(result.value.semanticTimeline.summary.hasSeparatedBass,false);assert.equal(result.value.semanticTimeline.summary.hasSeparatedAccompaniment,true);assert.equal(writes.length,1);
});
