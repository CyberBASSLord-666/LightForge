...
test('capped mix-percussion keeps recurrence and salience timeline bindings identical',()=>{
 ...
 const timeline=Timeline.build(music),salience=Salience.build(timeline),evidence=Recurrence.captureEvidence(music,timeline),sidecar=Recurrence.build(timeline,evidence);
 const capped=timeline.events.find(event=>event.analysisSource==='mix-feature-estimate');
 assert.ok(capped&&capped.salienceCap===.41&&capped.salience<=.41);
 assert.equal(Recurrence.timelineFingerprint(timeline),salience.timelineFingerprint);
 assert.equal(Recurrence.validateEvidence(evidence,timeline).valid,true);
 assert.equal(Recurrence.validate(sidecar,timeline,evidence).valid,true);
 assert.equal(Salience.validate(salience,timeline).valid,true);
 const changed=structuredClone(timeline);changed.events.find(event=>event.analysisSource==='mix-feature-estimate').salienceCap=.42;
 const changedSalience=Salience.build(changed),changedEvidence=Recurrence.captureEvidence(music,changed),changedSidecar=Recurrence.build(changed,changedEvidence);
 assert.equal(Recurrence.timelineFingerprint(changed),changedSalience.timelineFingerprint);
 assert.equal(Recurrence.timelineFingerprint(changed),changedSidecar.timelineFingerprint);
 assert.notEqual(Recurrence.timelineFingerprint(changed),Recurrence.timelineFingerprint(timeline));
 assert.equal(Recurrence.validateEvidence(evidence,changed).valid,false);
 assert.equal(Recurrence.validate(sidecar,changed,evidence).valid,false);
 assert.equal(Salience.validate(salience,changed).valid,false);
 assert.equal(Recurrence.validate(changedSidecar,changed,changedEvidence).valid,true);
 assert.equal(Salience.validate(changedSalience,changed).valid,true);
});
