...
module.exports=async function runWorker(directory,payload){
 let result,error;
 ...
 context.self=context;context.postMessage=data=>{if(data.type==='result')result=data.value;else if(data.type==='error')error=Error(data.message);};
