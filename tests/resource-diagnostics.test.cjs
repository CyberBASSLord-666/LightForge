...
async function restoredWorkerMessages({hostilePerformance=false,hostileNow=false,throwingNow=false,lateHostileNow=false}={}){
 ...
 const sandbox={
  ...
  // WorkerGlobalScope exposes Date directly. Keep that shape in the VM so the
  // shared clock exercises its real fallback rather than a missing-script stub.
  Date
 };
 sandbox.self=sandbox;
 const context=vm.createContext(sandbox);
 ...
 for(const file of ['diagnostic-clock.js','resource-diagnostics.js','telemetry.js'])vm.runInContext(fs.readFileSync(path.join(analysisRoot,file),'utf8'),context,{filename:file});
 vm.runInContext(fs.readFileSync(path.join(analysisRoot,'worker.js'),'utf8'),context,{filename:'worker.js'});
...
test('a hostile performance clock cannot abort a restored worker stage or alter its default result path',async()=>{
 for(const hostile of [{},{hostilePerformance:true},{hostileNow:true},{throwingNow:true},{lateHostileNow:true}]){
  ...
  if(hostile.lateHostileNow){
   ...
   assert.equal(result.profile.attributes.workerClockSource,'performance.now');
   ...
   assert.equal(result.profile.attributes.workerClockSource,'date.now');
