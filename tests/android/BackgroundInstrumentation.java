...
public final class BackgroundInstrumentation extends Instrumentation {
    ...
    private JSONObject waitTerminal(long timeout)throws Exception{
        ...
        while(SystemClock.elapsedRealtime()<until){JSONObject job=AnalysisJobStore.status(files);if(job!=null&&!AnalysisJobStore.active(job))return job;SystemClock.sleep(500);}
    ...
    private String fixture(String name)throws Exception{
        ...
        float[] pcm=new float[3*44100*2];for(int i=0;i<pcm.length;i++)pcm[i]=(float)(.18*Math.sin(2*Math.PI*220*(i/2)/44100));
        ...
        state.put("settings",new JSONObject().put("analysisQuality","balanced").put("dance","off").put("style","festival").put("stepMs",20).put("seed",2025));
    ...
    @Override public void onStart(){
        ...
        try{
            ...
            pass("Actual balanced neural analysis and choreography completed with the Activity destroyed, screen off and Doze forced with user-equivalent battery exemption; result was durably saved.");
            ...
            check(original.equals(AnalysisJobStore.hash(cancelProject)),"Timeout replaced saved project");
            pass("Android media-processing timeout callback stops promptly and leaves a retryable job with the previous show intact.");
            receipt.put("passed",true).put("checks",checks).put("sdk",Build.VERSION.SDK_INT).put("scope","Android emulator with actual foreground service, WebView/WASM models, screen-off execution, notification cancellation and timeout callback; not a physical phone or Tesla.");
