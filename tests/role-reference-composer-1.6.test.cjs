...
const errors=checks.filter(x=>!x.passed),receipt={release:E.version,passed:!errors.length,checkedAt:new Date().toISOString(),checks,results,source_hashes:Object.fromEntries(files.map(f=>[f,crypto.createHash('sha256').update(fs.readFileSync(path.join(root,f))).digest('hex')])),limitations:['Six short original MUSDB vocal stems and one controlled remix extracted by the production separator are acoustic references. This does not measure true beat accuracy, word timing, perceptual show quality or physical vehicle response.','A declared120BPM scaffold makes lighting translation reproducible; it is not asserted to be the tempo of these songs.','Source extraction confidence combines several estimates and is not a calibrated probability.','Expected physical lamp states are computed using the shipped preview simulator; independent fade-byte tests separately verify that simulator.']};fs.writeFileSync(path.join(root,'qa/release-'+E.version,'role-reference-composer-verification.json'),JSON.stringify(receipt,null,2)+'\n');if(errors.length)process.exitCode=1;
