...
const canonicalV8=duration=>{
 ...
 const evidence=Recurrence.captureEvidence({...value,chroma:Array(36).fill(.3),chromaStep:1},value.semanticTimeline),sidecar=Recurrence.build(value.semanticTimeline,evidence);
...
test('background runner checkpoints analysis and commits an actual compiled show without a studio document',async()=>{
 const progressEvents=[],events=[],request={analysisIdentity:'a'.repeat(64),version:1,projectId:'show-test',name:'Background',duration:2,settings:{dance:'off',vocalSemanticEnrichment:true,recurrenceAnalysis:true},music:null,needAnalysis:true};let done=false,saved;
 const context={URL,AbortController,DOMException,console,location:{href:'https://appassets.androidplatform.net/background/runner.html?job=job-one'},navigator:{},
  ...
  MusicAnalyzer:{analyze:async(url,options,progress)=>{assert.equal(options.projectId,'show-test');assert.equal(options.vocalSemanticEnrichment,true);assert.equal(options.recurrenceAnalysis,true);assert.equal(options.analysisIdentity,request.analysisIdentity);assert.match(url,/\/project\/show-test\/audio.wav$/);progress({progress:.5,stage:'separation',detail:'Analyzing',passageIndex:3,passageCount:8,passagesCompleted:2,restoredPassages:1,checkpointSaved:true});events.push('analysis');return structuredClone(music);}},
...
async function completedReconnect({acknowledged=false,hold=false,restoreError=null,terminalRejected=false,beginRejected=false,beginThrows=false,pulseRejected=false,ackConfirmationRejected=false,ackWriteFailures=0,nativeProjectPayload=false,fetchNeverResolves=false,deferInitialPreview=false,previewReady=undefined,visualCommitReady=true}={}){
 ...
 const coldBootstrap={projects:[],version:'2.2.4'},initialBootstrap={projects:[completed,lastSelected],lastProjectId:lastSelected.id,version:'2.2.4',backgroundJob:job};let bootstrapReads=0,initialBootstrapHeld=!!deferInitialPreview;
 // Startup probes and the restore-owned metadata refresh all need one immutable
 // completed-job snapshot. Release it only after the fixture has installed the
