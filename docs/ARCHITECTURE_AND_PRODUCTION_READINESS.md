...
| Tonal structure and phrases | `dsp.js` | Default analysis combines energy/spectral changes with multi-scale pitch-class novelty; measured phrase boundaries precede four-bar scaffolding. | Generic estimated sections and phrases, not chord names or verse/chorus classification; original beats and explicit silence remain authoritative. |
| Recurrence motifs | `recurrence.js`, worker recurrence stage, `motif-evolution.js` | New projects request a bounded, validated recurrence sidecar and motif evolution. Compatible saved analyses can be enriched without model inference. | Oversized evidence leaves recurrence explicitly unavailable while the complete base analysis remains usable. No conventional song labels or independently emitted vehicle commands. |
| Choreography | `music-cues.js`, `light-planner.js`, `movement-planner.js`, `show-engine.js`, `semantic-choreography.js`, `choreography-quality.js`, `sync-review.js` | Planners produce deterministic FSEQ frames for identical input/settings/seed. New projects request validated semantic, vocal and motif expression; historical missing flags remain false. Semantic collision allocation retains its separate opt-in. | Source/link validators remain mandatory. Software quality accounting does not establish detector correctness or listener preference. Selected supported vocal/bass attacks are protected from decorative density thinning. |
