...
**Source snapshot:** `8406acb89faa10ea486e1ec822379d2e177eccbd`  
...
| Feature reuse / FeatureStore | `wav-reader.js`, `dsp.js`, `work-store.js` | Individual analysis stages calculate and persist selected intermediate work and checkpoints. | There is **no standalone `feature-store.js` module at this revision** and no demonstrated generic shared STFT/mel/chroma broker across all analyzers. This is partial reuse, not completion of the planned shared FeatureStore architecture. |
| Stem routing | `separator-deux.js`, `separator-mdx.js`, `stem-cache.js`, optional Android `NativeDeux` / `NativeMdxTask` bridge | Studio uses Mel-Band RoFormer Deux; Balanced uses MDX-Net Voc FT. The standard semantic outputs are estimated `vocals` and `accompaniment`; native inference is used only when the integration provides it. | This is not a demonstrated four-semantic-stem system. Lead and backing singers remain combined. The original mix remains authoritative when a stem conflicts with it. |
| Rhythm | `worker.js`, `dsp.js`, bundled Beat This! assets | Rhythm/bar analysis runs in the staged worker; Studio and Balanced choose the documented full/compact Beat This! modes. | Beat/downbeat/meter values are estimates with confidence, not annotations or ground truth. Variable-tempo and irregular-meter quality requires locked-corpus evidence. |
...
| Percussion | `semantic-timeline.js` input contract | The timeline can ingest explicit, classed external/manual `percussionAnalysis` evidence with provenance and strict event labels. | No dedicated shipped drum/percussion detector is asserted here. Mix onsets, energy, or beats must not be promoted to labelled kick/snare/hat events by implication. |
...
| Choreography | `music-cues.js`, `light-planner.js`, `movement-planner.js`, `show-engine.js`, `choreography-quality.js`, `sync-review.js` | Planners turn music context into light/movement plans, then `ShowEngine` paints deterministic FSEQ frames for identical input/settings/seed. Quality and synchronization sidecars inspect the completed realization. | Quality sidecars are read-only diagnostics; they do not prove that the source music detector was correct or that a human prefers the show. Legacy analysis input retains a compatibility path. |
| Collision accounting | `light-planner.js`, `show-engine.js`, `sync-review.js` | Final synchronization reporting separates raw candidate rejection from recorded high-salience rescue and unresolved/loss accounting where planning metadata exists. | A low reported loss is not proof of musical correctness. Missing planner metadata makes some conflict assessments unassessed rather than silently successful. |
...
| Cache and recovery | `work-store.js`, `stem-cache.js`, `analyzer.js` | OPFS-backed stage/passages checkpoints, atomic writes, checksums, cancellation, and resumable staged analysis are enabled. Stem completion markers protect restoration before downstream use. | Cache is disposable and cannot be the export source of truth. A cache hit is not a speed claim; cache equivalence and cost still need controlled measurement. |
...
| Diagnostics, benchmark, and gate | `telemetry.js`, `analysis_benchmark_contract.py`, `locked_benchmark_runner.py`, `performance_quality_gate.py`, `differential_analysis.py` | Machine-readable stage diagnostics, contract validation, locked-corpus aggregation, baseline/candidate pairing, and differential tooling exist. The gate is fail-closed and compares provenance, conditions, timing, and quality metrics. | The committed policy contains `__configure_locked_corpus__`; it is intentionally not release evidence. A redacted template and passing unit tests do not equal a licensed corpus, a benchmark result, or a quality pass. |
...
At `8406acb`, LightForge has an offline staged analysis/choreography pipeline,
deterministic sequence generation, recoverable cache infrastructure, and a
fail-closed benchmark framework. It is **not yet supported by a configured
locked corpus and measured baseline/candidate release result**, and therefore
this snapshot must not claim production readiness, a quality-regression-free
optimization, or a 75% runtime reduction.
