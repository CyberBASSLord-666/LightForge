...
Release candidate: publication remains blocked after three failed or incomplete Android attempts. The latest completed-project restoration and preview startup fix passes all 347 Node and 69 Python tests locally, including 22 focused startup checks, with pixel-identical final renders in the tested Studio/Night scenes. A fresh complete Android run and a newly built, verified original-signed APK are required. Physical-phone and Tesla validation remain unperformed.
...
The preceding verification/build job passed host and Chromium pipeline gates before this startup fix. The [first Android run](qa/release-2.2.4/android-first-run/README.md) failed with a teardown ANR; the [second](qa/release-2.2.4/android-preview-first-frame-run/README.md) saved screen-off Studio work and then timed out on preview readiness. The [focused diagnostic run 34420755126](qa/release-2.2.4/android-readiness-diagnostic-run/README.md) reproduced the readiness timeout before the visual-state callback and separately completed six native diagnostic checks before a JavaScript callback timed out. Its observed pending composition/restoration and unloaded preview motivated the latest fix; this does not establish successful Android recovery yet. No LightForge ANR or out-of-memory failure was identified in that diagnostic run. Its temporary-signed APK is not release eligible. The new full Android run and signed build remain pending; earlier numerical results retain their original source-bound scope.
