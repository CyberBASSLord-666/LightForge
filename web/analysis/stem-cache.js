...
function header(samples){const b=new ArrayBuffer(44),v=new DataView(b);const text=(at,s)=>{for(let i=0;i<s.length;i++)v.setUint8(at+i,s.charCodeAt(i));};text(0,'RIFF');v.setUint32(4,36+samples*4,true);text(8,'WAVEfmt ');v.setUint32(16,16,true);v.setUint16(20,3,true);v.setUint16(22,1,true);v.setUint32(24,RATE,true);v.setUint32(28,RATE*4,true);v.setUint16(32,4,true);v.setUint16(34,32,true);text(36,'data');v.setUint32(40,samples*4,true);return new Uint8Array(b);}
...
async function prune(protect,needed=0){
 ...
 for await(const [key,handle]of dir.entries()){
  ...
  try{const file=await(await handle.getFileHandle('complete.json')).getFile(),meta=JSON.parse(await file.text());let size=0;for(const name of ['vocals.wav','accompaniment.wav'])size+=(await(await handle.getFileHandle(name)).getFile()).size;total+=size;entries.push({key,size,time:meta.createdAt||file.lastModified});}
...
async function create(key,sampleCount,config,sourceId=''){
 ...
 const expected=Math.ceil(sampleCount/2),needed=88+expected*8;
 ...
 const writers={};
 try{for(const name of ['vocals','accompaniment']){const stream=await(await dir.getFileHandle(name+'.wav',{create:true})).createWritable();await stream.write(header(expected));writers[name]=new DownsampleWriter(stream,config.resampleHalfFIR,sampleCount);}}
 catch(e){for(const writer of Object.values(writers))await writer.stream.abort().catch(()=>{});await discard(key);throw e;}
 let finished=false;
 return{key,async append(chunk){if(finished)throw Error('Analysis audio is already complete.');if(chunk.sampleRate!==44100)throw Error('Separated audio has an unsupported sample rate.');if(!(chunk.vocals instanceof Float32Array)||!(chunk.accompaniment instanceof Float32Array)||chunk.vocals.length!==chunk.accompaniment.length)throw Error('Separated audio layers must have matching sample counts.');await writers.vocals.push(chunk.vocals,chunk.startSample);await writers.accompaniment.push(chunk.accompaniment,chunk.startSample);},async finish(){for(const writer of Object.values(writers))await writer.finish();const meta={version:1,key,createdAt:Date.now(),sampleRate:RATE,samples:expected,duration:sampleCount/44100,sourceId};const stream=await(await dir.getFileHandle('complete.json',{create:true})).createWritable();await stream.write(JSON.stringify(meta));await stream.close();finished=true;return meta;},async abort(){for(const writer of Object.values(writers))await writer.stream.abort().catch(()=>{});await discard(key);}};
}
...
root.LightForgeStemCache={create,files,readers,discard,prune,validKey,header,DownsampleWriter};
