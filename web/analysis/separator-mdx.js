...
async function create(options={}){
 ...
 const timed=(name,fn)=>options.telemetry?.measure?options.telemetry.measure('performance.'+name,fn,{component:'mdx'}):fn();
 const timedAsync=(name,fn)=>options.telemetry?.measureAsync?options.telemetry.measureAsync('performance.'+name,fn,{component:'mdx',runtime:'onnxruntime-web-wasm'}):fn();
 ...
 async function ensureSession(){
  ...
  if(!sessionPromise)sessionPromise=(async()=>{options.onProgress?.({stage:'loading',progress:0,message:'Loading studio vocal separation'});try{return await timedAsync('model_initialization',()=>ort.InferenceSession.create(base+manifest.file,sessionOptions));}catch(e){throw new Error('Could not load studio vocal separation. Free device memory and retry. '+(e?.message||e));}})();
 ...
 async function runWasm(encoded){
  ...
  try{tensor=new ort.Tensor('float32',encoded,[1,4,BINS,FRAMES]);result=await timedAsync('model_inference',()=>active.run({[active.inputNames[0]]:tensor}));prediction=timed('postprocessing',()=>Float32Array.from(result[active.outputNames[0]].data));return prediction;}
 ...
 return{manifest,async process(readStereo44100,sampleCount,onChunk,onProgress){
  ...
  for(let start=0;start<sampleCount;start+=STRIDE){const keep=Math.min(CORE,sampleCount-start),last=sampleCount-start<=CORE,stereo=await readStereo44100(start-TRIM,INPUT_LENGTH);const peak=timed('preprocessing',()=>{let peak=0;for(let c=0;c<2;c++)for(let i=0;i<stereo[c].length;i++)peak=Math.max(peak,Math.abs(finite(stereo[c][i])));return peak;});let decoded;
   ...
   if(restored){decoded=cached[0];restoredPassages++;report('Restoring a completed vocal passage',.9);}
   else if(peak<1e-7)decoded=new Float32Array(INPUT_LENGTH);else{const encoded=timed('preprocessing',()=>frontend.encode(stereo)),nativeAvailable=typeof options.nativePredict==='function';let prediction;
    ...
    if(denoise){report('Refining vocal isolation with a second listening pass',.5);const reversed=timed('preprocessing',()=>{const reversed=encoded.slice();for(let i=0;i<reversed.length;i++)reversed[i]=-reversed[i];return reversed;});const reverse=nativeAvailable?await runNative(reversed,nativeProgress):await runWasm(reversed);
     if(nativeAvailable){passes++;nativePasses++;}else{passes++;wasmPasses++;}
     timed('postprocessing',()=>{for(let i=0;i<prediction.length;i++)prediction[i]=(prediction[i]-reverse[i])*.5;});
    }
    decoded=timed('postprocessing',()=>frontend.decode(prediction,manifest.compensate));await options.checkpoint?.writeFloats(checkpointName,[decoded]);}
   const {vocals,count,outputVocals,accompaniment}=timed('postprocessing',()=>{const vocals=decoded.slice(TRIM,TRIM+keep),mixture=new Float32Array(keep);for(let i=0;i<keep;i++)mixture[i]=(finite(stereo[0][i+TRIM])+finite(stereo[1][i+TRIM]))*.5;if(pending){for(let i=0;i<Math.min(OVERLAP,keep,pending.length);i++){const w=.5-.5*Math.cos(Math.PI*(i+.5)/OVERLAP);vocals[i]=pending[i]*(1-w)+vocals[i]*w;}}
   const count=last?keep:STRIDE,outputVocals=vocals.slice(0,count),accompaniment=new Float32Array(count);for(let i=0;i<count;i++)accompaniment[i]=mixture[i]-outputVocals[i];return {vocals,count,outputVocals,accompaniment};});if(start!==emitted)throw new Error('Vocal-separation timing continuity was lost.');await onChunk({vocals:outputVocals,accompaniment,startSample:start,sampleRate:RATE});emitted+=count;chunks++;pending=timed('postprocessing',()=>last?null:vocals.slice(STRIDE));if(last)break;await retireSession();await new Promise(resolve=>setTimeout(resolve,0));
  }
