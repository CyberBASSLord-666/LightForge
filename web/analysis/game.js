...
async function create({ort,baseUrl,onProgress=()=>{}}){
 const sessions={},manifest=await(await fetch(new URL('manifest.json',baseUrl))).json();
 try{for(const name of ['encoder','dur2bd','segmenter','bd2dur','estimator']){onProgress('Loading '+name);sessions[name]=await ort.InferenceSession.create(new URL(name+'.onnx',baseUrl).href,{executionProviders:['wasm'],graphOptimizationLevel:'all',enableCpuMemArena:false,enableMemPattern:false});}}
 catch(e){await Promise.all(Object.values(sessions).map(s=>s.release()));throw e;}
 const tensor=(type,data,dims)=>new ort.Tensor(type,data,dims);
 async function infer(pcm,language,seed){
  const owned=[],own=t=>(owned.push(t),t);let encoded,known,previous,timing,estimated;
  try{
   encoded=await sessions.encoder.run({waveform:own(tensor('float32',pcm,[1,pcm.length])),duration:own(tensor('float32',Float32Array.of(pcm.length/RATE),[1]))});
 ...
 return {manifest,infer,async process(read,total,{language=0,onProgress=()=>{}}={}){
  if(![0,1,2,3,4].includes(language)||!Number.isSafeInteger(total)||total<1)throw Error('Invalid transcription request.');
  const notes=[],duration=total/RATE,chunks=Math.ceil(duration/CORE);
  for(let i=0;i<chunks;i++){
   ...
   if(peak>1e-5)for(const n of await infer(pcm,language,(2025+i*104729)>>>0)){
 ...
 },async release(){await Promise.all(Object.values(sessions).map(s=>s.release()));}};
}
