...
(function(root){'use strict';
 ...
 function build(music){
  ...
  function add(type,time,durationValue,source,attributes={}){
   ...
   for(const key of ['kind','midi','frequency','articulation','recurrenceGroup','repetitionIndex','label','estimated','sourceSeparated','manual'])if(attributes[key]!==undefined)event[key]=attributes[key];
   events.push(event);
  }
  ...
  for(const note of music.bassNotes||[])if(validSpan(note,duration))add('bass_note',note.start,note.end-note.start,'bass',{confidence:note.confidence,strength:note.strength,midi:note.midi,frequency:note.frequency,sourceSeparated:note.sourceSeparated===true||music.bassAnalysis?.sourceSeparated===true,estimated:note.estimated!==false});
  for(const phrase of music.bassAnalysis?.phrases||[])if(validSpan(phrase,duration))add('bass_phrase',phrase.start,phrase.end-phrase.start,'bass',{confidence:phrase.confidence,strength:phrase.strength,midi:phrase.midi,frequency:phrase.frequency,sourceSeparated:phrase.sourceSeparated===true||music.bassAnalysis?.sourceSeparated===true,estimated:phrase.estimated!==false});
  ...
  return {schemaVersion:1,clock:'original-decoded-audio',duration,events,summary:{eventCount:events.length,countByTier,hasSeparatedVocals:vocals.sourceSeparated===true,hasSeparatedBass:music.bassAnalysis?.sourceSeparated===true,lyricsAligned:music.roleAnalysis?.lyricsAligned===true}};
 }
 function validate(timeline){const errors=[];if(!timeline||timeline.schemaVersion!==1||!finite(timeline.duration)||timeline.duration<=0||!Array.isArray(timeline.events))errors.push('Invalid semantic timeline envelope.');
