...
(function(root){
  ...
  function resolveTimelineApi(override){
    if(override)return override;
    if(root.LightForgeSemanticTimeline)return root.LightForgeSemanticTimeline;
    if(typeof require==='function')try{return require('../analysis/semantic-timeline.js');}catch(_){return null;}
    return null;
  }
  function exactLink(sidecar,links,timeline,api){
    ...
    try{return canonical(recomputed)===canonical(links)?recomputed:null;}catch(_){return null;}
  }
  function validTimeline(timeline,sidecar,api){
    // Do not duplicate the timeline contract here. The bridge must use the
    // exact v2/original-clock validator that binds every other semantic
    // sidecar; future integrity fields then fail closed automatically.
    if(!timeline||timeline.schemaVersion!==2||timeline.clock!=='original-decoded-audio'||!finite(timeline.duration)||!near(timeline.duration,sidecar.duration,1e-6))return false;
    let check;
    try{check=api&&typeof api.validate==='function'?api.validate(timeline):null;}catch(_){return false;}
  ...
  function create(input,options={}){
    ...
    const sidecar=music.vocalSemantics,links=music.vocalSemanticLinks,api=resolveApi(options.api),timelineApi=resolveTimelineApi(options.timelineApi);
    ...
    if(!validTimeline(music.semanticTimeline,sidecar,timelineApi))return inactive(true,'invalid-semantic-timeline');
    let validation;
