...
const canvas=$('scoreCanvas'),ctx=canvas.getContext('2d'),audio=$('audio');let zoom=1,lastMusic=null,lastSettings=null,lastWidth=0,from=0,span=1,pending=false;
function draw(){pending=false;const music=app.state.music;timeline.hidden=!music;if(!music||!ctx)return;
 ...
 for(const p of music.vocals?.phrases||[])band(p.start,p.end,39,28,'#33373e');
 ...
 lastMusic=music;lastSettings=app.state.settings;lastWidth=width;
}
...
audio.addEventListener('timeupdate',schedule);audio.addEventListener('seeked',schedule);window.addEventListener('resize',schedule);
