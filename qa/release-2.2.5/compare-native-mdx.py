...
import datetime, hashlib, json, math, os, platform, re, subprocess, sys, tempfile
import numpy as np
ROOT=Path(__file__).resolve().parents[2]
OUT=ROOT/'qa/release-2.2.5'
HISTORICAL_OUT=ROOT/'qa/release-2.2.4'
WORK=Path(os.environ.get('LIGHTFORGE_MDX_COMPARISON_DIR',ROOT.parent/'native-mdx-comparison-2.2.5'))
...
if EVIDENCE_SESSION is not None:
    if not re.fullmatch(r'[0-9a-f]{32,128}\Z',EVIDENCE_SESSION):
        raise ValueError('Invalid LIGHTFORGE_EVIDENCE_SESSION')
...
def main():
    ...
    try:
        ...
        receipt['completedAt']=datetime.datetime.now(datetime.timezone.utc).isoformat();write_atomic(gate,receipt)
