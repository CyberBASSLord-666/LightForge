...
(async()=>{
 ...
 const receipt={release:'2.2.5',passed:false,checks:[],errors:[],source_hashes:{},scope:'Desktop Chromium with actual WebGL and web workers; simulated Android bridge. No Android device or Tesla.'};
 ...
 try{
  ...
  await page.addInitScript(()=>{
   ...
   window.Android={pickAudio(){},getBootstrap:()=>JSON.stringify({projects:[project],lastProjectId:project.id,version:'2.2.5'}),saveProject:(id,body)=>{localStorage.setItem('qa-saved',body);return true;},beginExport:metadata=>{window.qaExport={metadata:JSON.parse(metadata),chunks:[]};return 'qa-export';},appendExport:(id,data)=>{window.qaExport.chunks.push(data);return true;},finishExport:()=>window.onNativeEvent('exported',{name:'QA export',id:'qa-export'})};
  });
  ...
  assert.equal(crypto.createHash('sha256').update(bytes.subarray(offset)).digest('hex'),frameHash);assert.equal(exp.metadata.projectState.provenance.app,'2.2.5');assert.equal(exp.metadata.validation.synchronization.manual[0].status,'matched');
