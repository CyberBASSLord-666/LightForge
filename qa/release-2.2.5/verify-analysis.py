...
ASSET_MANIFEST_SHA256 = '2afc522a366ed00b1d47bd85c5f97d7e1b75b239d8e1c8412fbf8d28ad1377d9'
...
def verify_historical_comparison(root, hashes):
    ...
    Current 2.2.5 source bytes are bound by fresh session receipts, not by this
...
def verify_profile_equivalence(root, hashes, comparison, evidence_session=EVIDENCE_SESSION_UNSET):
    # The paired proof is fresh v2.2.5 evidence; v2.2.4 is only recorded context.
...
def verify_clock(root, hashes, evidence_session=None):
    ...
    require(result.get('release') == RELEASE and result.get('passed') is True and not result.get('errors'),
            'Fresh 2.2.5 source-clock check did not pass')
...
def verify_release(root=ROOT):
    ...
    require(json.loads(version_path.read_text()) == {'name': RELEASE, 'code': RELEASE_CODE},
            'This evidence protocol belongs only to 2.2.5 / 20205')
    ...
    receipt = {
        ...
        'checks': [
            ...
            'Fresh 2.2.5 source-clock execution preserves all 932143 samples across four overlapping windows, including the final odd sample.',
