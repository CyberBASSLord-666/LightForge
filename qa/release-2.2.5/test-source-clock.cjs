...
const sources=['web/analysis/separator-deux.js','web/analysis/dsp.js','web/analysis/models/deux/manifest.json','qa/release-2.2.5/test-source-clock.cjs','version.json'];
const hashes=()=>Object.fromEntries(sources.map(p=>[p,sha(path.join(root,p))])),sourceHashes=hashes();
const receiptPath=path.join(__dirname,'source-clock-verification.json');
const writeReceipt=value=>{const temporary=receiptPath+'.tmp';fs.writeFileSync(temporary,JSON.stringify(value,null,2)+'\n');fs.renameSync(temporary,receiptPath);};
writeReceipt({release:'2.2.5',passed:false,errors:['Source-clock verification has not completed.'],source_hashes:sourceHashes});
(async()=>{
 assert.deepEqual(JSON.parse(fs.readFileSync(root+'/version.json')), {name:'2.2.5',code:20205}, 'This source-clock gate is for 2.2.5 / 20205');
 ...
 assert.deepEqual(hashes(),sourceHashes,'Source changed while verifying its clock');const receipt={release:'2.2.5',passed:true,errors:[],source_hashes:sourceHashes,scope:'Neutral masks and fake graph sessions; actual production STFT/scatter/ISTFT and overlapping chunk scheduler. No neural inference.',samples:total,chunks:result.chunks,maxAbsError:maximum,contiguousSourceSamples:true,monotonicProgress:true};writeReceipt(receipt);console.log(receipt);
})().catch(e=>{writeReceipt({release:'2.2.5',passed:false,errors:[String(e.stack||e)],source_hashes:sourceHashes});console.error(e);process.exitCode=1;});
