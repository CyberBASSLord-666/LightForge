...
with zipfile.ZipFile(ARCHIVE) as z:
 for slug,split,title in TRACKS:
  ...
  entry={'id':slug,'title':title,'datasetSplit':split,'license':meta['License'],'source':meta['Source'],'duration':duration,'sampleRate':rate,'channels':2,'archiveMember':member,'pcmSHA256':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in OUT.glob(slug+'-*.wav')},'controlledEdit':{'vocalWindows':[[1.25,3.15],[4.2,6.0]],'edgeRampSeconds':.01,'definition':'Original vocals gated in time and summed with original drums, bass and other stems; this does not assert continuous singing throughout either window.'}}
  receipt['tracks'].append(entry)
  print(slug,duration,flush=True)
(OUT.parent/'musdb-fixture-provenance.json').write_text(json.dumps(receipt,indent=2)+'\n')
