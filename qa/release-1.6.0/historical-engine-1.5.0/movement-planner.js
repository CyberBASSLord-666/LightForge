/* LightForge 1.5 — phrase-aware choreography for slow physical actuators.
...
(function(root){
  'use strict';
  const VERSION='1.5.0', COMMAND={Open:63,Dance:127,Close:191};
  ...
  function plan(music,settings,profile){
    ...
    if(vocalFocus>0&&music.vocals?.available&&music.vocals.presence!=='not_detected')for(const p of music.vocals.phrases||[]){
      ...
      offer(p.start,'estimated vocal entrance',.66+.29*vocalFocus,p.confidence,'vocals',p.end);
