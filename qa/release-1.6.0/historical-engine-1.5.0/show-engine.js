...
(function (root) {
  ...
  function normalizeMusic(music, warnings) {
    ...
    const vocals={available:voice.available===true,presence:['detected','not_detected','uncertain'].includes(voice.presence)?voice.presence:'uncertain',method:String(voice.method||'unavailable').slice(0,200),model:voice.model||null,confidence:clamp(finite(voice.confidence)?voice.confidence:0,0,1),phrases:roleEvents(voice.phrases),accents:Array.from(voice.accents||[]).filter(x=>x&&finite(x.time)&&x.time>=0&&x.time<duration&&finite(x.confidence)&&finite(x.strength)).map(x=>({time:x.time,confidence:clamp(x.confidence,0,1),strength:clamp(x.strength,0,1),estimated:true})).sort((a,b)=>a.time-b.time),envelope:envelope(voice.envelope,vocalStep),envelopeStep:vocalStep,timing:voice.timing||{}};
  ...
  function generate(music, settings) {
    ...
    const warnings=[],s=normalizeSettings(settings),m=correctRhythm(normalizeMusic(music,warnings),s);
  ...
  function paintInterior(show,m,s,ctx){
    const {frames,frameCount,stepMs}=show,step=stepMs/1000,palette=PALETTES[s.palette];let bi=0,voiceLevel=0,bassLevel=0;
    for(let f=0;f<frameCount-1;f++){
      ...
      // Smooth region-gated estimates: these are phrase dynamics, never a lyric
      // transcript or isolated syllable envelope. The beat layer stays present.
      const targetVoice=voice&&s.vocalFocus>0?ctx.roleEnvelope(m.vocals,time,voice.strength):0,targetBass=bass&&s.bassFocus>0?ctx.roleEnvelope(m.bassAnalysis,time,bass.strength):0;
      voiceLevel+=(targetVoice-voiceLevel)*Math.min(1,step/(targetVoice>voiceLevel?.10:.22));bassLevel+=(targetBass-bassLevel)*Math.min(1,step/(targetBass>bassLevel?.025:.10));
      for(let zone=0;zone<6;zone++){
        ...
        const roleGain=focusedVoice?clamp(gain*(1-.35*s.vocalFocus)+voiceLevel*s.vocalFocus*.58*(.3+.7*scene.intensity)):focusedBass?clamp(gain*(1-.35*s.bassFocus)+bassLevel*s.bassFocus*.62*(.3+.7*scene.intensity)):gain;
        const start=groups.interior[zone]-1;for(let c=0;c<3;c++)frames[f*200+start+c]=Math.round((color[c]*(1-blend)+next[c]*blend)*roleGain*balance);
      }
    }
  }
