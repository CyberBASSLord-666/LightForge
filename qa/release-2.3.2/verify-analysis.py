#!/usr/bin/env python3
"""Bind fresh 2.3.2 source-clock and analysis evidence to a historical anchor.
...
inventory, and a fresh source-clock regression for v2.3.2. It does not claim a
...
# Reviewed 2.3.2 inventory: the exact complete asset manifest remains pinned.
# Original model/runtime assets and all source-bound verification gates remain mandatory.
ASSET_MANIFEST_SHA256 = 'c0dc1d56b53cf7f316697d45f0a2cb5a63c4bf7068ba89829619cadfe46ff790'
...
COMPARISON_SOURCES = {
    ...
    HISTORICAL_OUT + 'compare-native-runtime.py',
}
...
PROFILE_EQUIVALENCE_SOURCES = {
    ...
    'web/demo/glass-castle.wav', HISTORICAL_OUT + 'native-runtime-comparison-verification.json',
    HISTORICAL_OUT + 'compare-native-runtime.py', OUT + 'verify-native-inference-profile.py', OUT + 'verify-analysis.py',
}
CLOCK_SOURCES = {
...
def verify_fresh_receipt(root, relative, hashes, session, label, sources):
    ...
    require(result.get('release') == RELEASE and result.get('passed') is True and
...
def verify_historical_comparison(root, hashes):
    """Bind the v2.2.4 runtime result only as immutable historical context.

    Current 2.3.2 source bytes are bound by fresh session receipts, not by this
    earlier comparison's source inventory or version metadata.
    """
    path = bind(root, HISTORICAL_OUT + 'native-runtime-comparison-verification.json',
                COMPARISON_SHA256, hashes)
    result = json.loads(path.read_text())
    require(result.get('release') == '2.2.4' and result.get('passed') is True and
            not result.get('failure'), 'Historical native runtime comparison is invalid')
    require(result.get('start_sample') == -66150 and result.get('sample_rate') == 44100 and
            result.get('samples_per_stem') == SAMPLES and result.get('thresholds') == THRESHOLDS,
            'Historical native runtime comparison coverage changed')
    runs = result.get('runs', [])
    require(len(runs) == 2 and [run.get('version') for run in runs] == ['1.23.2', '1.25.1'],
            'Historical native runtime comparison versions changed')
    for run, expected, size in zip(runs, [OLD_RUNTIME_SHA256, NEW_RUNTIME_SHA256], [75255976, 41804437]):
        require(run.get('runtime_jar_sha256') == expected and run.get('runtime_jar_bytes') == size and
                run.get('samplesPerStem') == SAMPLES and run.get('output_bytes') == 2 * SAMPLES * 4,
                'Historical native runtime comparison binding changed')
    stems = result.get('comparison', [])
    require(len(stems) == 2 and [stem.get('stem') for stem in stems] == ['vocals', 'accompaniment'],
            'Historical native runtime comparison stem coverage changed')
    for stem in stems:
        require(stem.get('finite') is True and stem.get('samples') == SAMPLES,
                'Historical native runtime comparison contains invalid samples')
        for key, limit in THRESHOLDS.items():
            value = stem.get(key)
            require(finite_number(value) and 0 <= value <= limit,
                    'Historical native runtime numerical threshold failed: ' + key)
    model_hashes = result.get('model_asset_hashes')
    require(isinstance(model_hashes, dict) and len(model_hashes) == 27 and
            all(isinstance(name, str) and name.startswith('web/analysis/models/deux/') and
                isinstance(value, str) and re.fullmatch(r'[0-9a-f]{64}', value)
                for name, value in model_hashes.items()),
            'Historical native runtime model inventory changed')
    return result


def verify_profile_equivalence(root, hashes, comparison, evidence_session=EVIDENCE_SESSION_UNSET):
    # The paired proof is fresh v2.3.2 evidence; v2.2.4 is only recorded context.
    ...
    require(set(result) == expected_keys and result.get('schema') == PROFILE_EQUIVALENCE_SCHEMA and
            result.get('release') == RELEASE and result.get('passed') is True,
...
def verify_clock(root, hashes, evidence_session=None):
    ...
    require(result.get('release') == RELEASE and result.get('passed') is True and not result.get('errors'),
            'Fresh 2.3.2 source-clock check did not pass')
...
MDX_SOURCES = {
    ...
    HISTORICAL_OUT + 'initial-mdx-absolute-only/compare-native-mdx.py',
    HISTORICAL_OUT + 'initial-mdx-absolute-only/native-mdx-comparison-verification.json',
    HISTORICAL_OUT + 'initial-mdx-absolute-only/native-mdx-comparison.log',
    HISTORICAL_OUT + 'revised-mdx-first-run/compare-native-mdx.py',
    HISTORICAL_OUT + 'revised-mdx-first-run/mdx_numeric.py',
    HISTORICAL_OUT + 'revised-mdx-first-run/native-mdx-comparison-verification.json',
    HISTORICAL_OUT + 'revised-mdx-first-run/native-mdx-comparison.log',
}


def verify_mdx(root, hashes, profile_equivalence, evidence_session=None):
    relative = OUT + 'native-mdx-comparison-verification.json'
    require(evidence_session is not None, 'Fresh MDX comparison requires an evidence session')
    path, result = bind_session_receipt(root, relative, '0' * 64, hashes,
    ...
    require(revision.get('prior_revised_failure_path') == HISTORICAL_OUT + 'revised-mdx-first-run/native-mdx-comparison-verification.json', 'Revised MDX failure provenance missing')
    ...
    require(digest(file(root, HISTORICAL_OUT + 'revised-mdx-first-run/compare-native-mdx.py')) == previous['source_hashes'][HISTORICAL_OUT + 'compare-native-mdx.py'], 'Earlier MDX comparator differs from its measured source')
    review = result.get('criterion_review', {})
    require(review.get('initial_failure_path') == HISTORICAL_OUT + 'initial-mdx-absolute-only/native-mdx-comparison-verification.json', 'Initial MDX criterion failure provenance missing')
    ...
    require(review.get('initial_comparator_path') == HISTORICAL_OUT + 'initial-mdx-absolute-only/compare-native-mdx.py', 'Initial MDX comparator provenance missing')
    require(review.get('initial_comparator_sha256') == initial['source_hashes'][HISTORICAL_OUT + 'compare-native-mdx.py'], 'Initial MDX comparator differs from the failed measurement')
...
def verify_downstream(root, hashes, mdx, evidence_session=None):
    ...
    require(evidence_session is not None, 'Fresh downstream comparison requires an evidence session')
    path, result = bind_session_receipt(root, relative, '0' * 64, hashes,
...
def verify_native_game(root, hashes, evidence_session):
    # Import this release's reviewed verifier, never a receipt-provided path.
    relative = OUT + 'native_game_evidence.py'
    spec = importlib.util.spec_from_file_location('lightforge_native_game_evidence_2_3_2', file(root, relative))
    require(spec is not None and spec.loader is not None, 'Native GAME evidence validator is missing')
    verifier = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(verifier)
    receipt = verify_fresh_receipt(root, OUT + 'native-game-verification.json', hashes,
        evidence_session, 'Native GAME', verifier.SOURCES)
    # Match the producer's strict parser, including duplicate-key and exponent
    # overflow rejection, before accepting any nested evidence metadata.
    require(verifier.load(file(root, OUT + 'native-game-verification.json')) == receipt,
            'Native GAME receipt parser disagreement')
    verifier.verify_receipt(root, receipt, evidence_session, hashes)
    return receipt
...
def verify_release(root=ROOT):
    ...
    require(json.loads(version_path.read_text()) == {'name': RELEASE, 'code': RELEASE_CODE},
            'This evidence protocol belongs only to 2.3.2 / 20302')
    evidence_session = current_evidence_session()
    require(evidence_session is not None,
            'Current 2.3.2 release verification requires a fresh evidence session')
    ensure_browser_source_inventory()
    ...
    browser = analysis_browser = background_ui = restore_preview = None
    if evidence_session is not None:
        ...
        background_ui = verify_background_ui_receipt(root, hashes, evidence_session)
        restore_preview = verify_restore_preview_receipt(root, hashes, evidence_session)
    ...
    native_game = verify_native_game(root, hashes, evidence_session)
    ...
    receipt = {
        'release': RELEASE, 'passed': True, 'errors': [], 'source_hashes': hashes,
        ...
        'scope': 'Pinned matched Linux/JVM full-passage Deux runtime comparison, a mandatory same-session exact-byte unprofiled-versus-profiled Deux observer proof, fresh production native MDX versus CPU WASM comparison, fresh actual production NativeGame versus WASM final-transcription and checkpoint proof on complete public demo and licensed Falcon sources, fresh production source-clock regression, and exact verification of all bundled analysis assets. This is not a new corpus accuracy benchmark, Android/ARM64 crash reproduction, phone performance result, or validation of background compatibility fallback.',
        'checks': [
            ...
            'Fresh 2.3.2 source-clock execution preserves all 932143 samples across four overlapping windows, including the final odd sample.',
            ...
            'Fresh production NativeGame and original WASM execute every fixed public demo/Falcon passage with unchanged graphs, eight steps and exact context/seeds. Retained unrounded notes are reprocessed through the actual consumer and native checkpoint path; final output must match while pitch differences and the absence of native tensor observations remain explicit.',
        ...
        'historical_native_runtime_comparison': {'path': HISTORICAL_OUT + 'native-runtime-comparison-verification.json',
        ...
        'fresh_native_game': {'path': OUT + 'native-game-verification.json',
            'sha256': hashes[OUT + 'native-game-verification.json'],
            'scope': native_game['scope'], 'runs': native_game['runs']},
    ...
    if evidence_session is not None:
        receipt['session_evidence_receipts'] = {
            ...
            'background_ui': {'path': OUT + 'background-ui-verification.json',
                              'sha256': hashes[OUT + 'background-ui-verification.json'],
                              'evidenceSession': evidence_session,
                              'scope': background_ui.get('scope')},
            'restore_preview': {'path': OUT + 'restore-preview-verification.json',
                                'sha256': hashes[OUT + 'restore-preview-verification.json'],
                                'evidenceSession': evidence_session,
                                'scope': restore_preview.get('scope')},
            ...
            'native_mdx_downstream': {'path': OUT + 'native-mdx-downstream-verification.json',
                                      ...
                                      'evidenceSession': evidence_session},
            'native_game': {'path': OUT + 'native-game-verification.json',
                            'sha256': hashes[OUT + 'native-game-verification.json'],
                            'evidenceSession': evidence_session},
...
def main():
    ...
    write_atomic(output, {'release': RELEASE, 'passed': False, 'errors': []})
    try:
        receipt = verify_release()
    except Exception as error:
        # Never leave a stale passing receipt after a failed refresh.
        receipt = {'release': RELEASE, 'passed': False, 'errors': [str(error)]}
