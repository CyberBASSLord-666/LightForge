#!/usr/bin/env python3
"""Bind fresh 2.3.0 source-clock and analysis evidence to a historical anchor.
...
inventory, and a fresh source-clock regression for v2.3.0. It does not claim a
...
# Reviewed 2.3.0 inventory: only DSP, optional recurrence handling and README
# differ from the prior inventory. Original model/runtime assets are unchanged.
ASSET_MANIFEST_SHA256 = 'b0fead7f348f4405be53f161f45fdbc79663e4cd9e8cbe0d75290adea713a039'
...
def verify_historical_comparison(root, hashes):
    ...
    Current 2.3.0 source bytes are bound by fresh session receipts, not by this
...
def verify_profile_equivalence(root, hashes, comparison, evidence_session=EVIDENCE_SESSION_UNSET):
    # The paired proof is fresh v2.3.0 evidence; v2.2.4 is only recorded context.
...
def verify_clock(root, hashes, evidence_session=None):
    ...
    require(result.get('release') == RELEASE and result.get('passed') is True and not result.get('errors'),
            'Fresh 2.3.0 source-clock check did not pass')
...
def verify_release(root=ROOT):
    ...
    require(json.loads(version_path.read_text()) == {'name': RELEASE, 'code': RELEASE_CODE},
            'This evidence protocol belongs only to 2.3.0 / 20300')
    ...
    receipt = {
        ...
        'checks': [
            ...
            'Fresh 2.3.0 source-clock execution preserves all 932143 samples across four overlapping windows, including the final odd sample.',
