...
function sourceNames(){
 const inventory=JSON.parse(fs.readFileSync(path.join(root,'qa/release-2.3.0/browser-source-inventory.json'),'utf8'));
...
(async()=>{
 ...
 const receipt={release:'2.3.0',passed:false,errors:[],checks:[],source_hashes:{},scope:'Actual Chromium public MusicAnalyzer with production Deux, GAME Large, Beat This and Frame-MN10. Test controls and original-stem reference scoring are independent of model inputs. No physical Android or Tesla.'};
 ...
 try{
  ...
  await page.goto('http://127.0.0.1:'+server.address().port);await page.waitForFunction(()=>!!window.MusicAnalyzer&&!!window.LightForgeApp);assert.equal(await page.evaluate(()=>crossOriginIsolated),true);
  const analysisStartedAt=process.hrtime.bigint();
  const result=await page.evaluate(async()=>{
   const settings=LightForgeApp.state.settings;const progress=[];let last=-1;const music=await MusicAnalyzer.analyze('/qa/falcon.wav',{projectId:'model-runtime',analysisQuality:'precision',vocalSemanticEnrichment:settings.vocalSemanticEnrichment===true,recurrenceAnalysis:settings.recurrenceAnalysis===true},p=>{progress.push(p);if(Math.floor(p.progress*20)!==last){last=Math.floor(p.progress*20);console.log(p.stage+' · '+Math.round(p.progress*100)+'%');}});
   ...
   const vocalValid=LightForgeVocalSemantics.validate(music.vocalSemantics,{duration:music.duration,vocals:music.vocals}).valid,recurrenceValid=LightForgeRecurrence.validate(music.recurrenceSidecar,music.semanticTimeline,music.recurrenceEvidence).valid;
   const vocalLinkCurrent=JSON.stringify(LightForgeVocalSemantics.linkTimeline(music.vocalSemantics,music.semanticTimeline))===JSON.stringify(music.vocalSemanticLinks);
   return {music,vocalValid,vocalLinkCurrent,recurrenceValid,cacheHash:hash,fullVoiceSamples:first.length,progressMonotonic:progress.every((p,i)=>!i||p.progress>=progress[i-1].progress-1e-9)};
  });
  assert.equal(result.vocalValid,true);assert.equal(result.vocalLinkCurrent,true);assert.equal(result.recurrenceValid,true);assert.equal(result.music.recurrenceAnalysis?.enabled,true);
  ...
  assert.deepEqual(Object.keys(performance.resources.pipeline.stages).sort(),['bass','recurrence','rhythm','separation','voice']);
  ...
  const composition=await page.evaluate(async()=>{const settings={...LightForgeApp.state.settings,dance:'off'},first=await ShowCompiler.generate(window.modelResult,settings),restored=await ShowCompiler.restore(first.compiled,window.modelResult,settings);const hash=Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',first.show.frames)),x=>x.toString(16).padStart(2,'0')).join('');return {valid:first.show.validation.valid,hash,savedHash:first.compiled.sha256,restoredHash:restored.compiled.sha256,frameCount:first.show.frameCount,expression:{vocal:first.show.choreography.vocalChoreography,semantic:first.show.choreography.semanticStrategy,motif:first.show.choreography.motifEvolution},synchronization:first.show.synchronization};});assert.ok(composition.valid);assert.equal(composition.expression.vocal?.active,true);assert.equal(composition.expression.semantic?.active,true);assert.equal(composition.expression.motif?.requested,true);assert.equal(composition.hash,composition.savedHash);assert.equal(composition.restoredHash,composition.savedHash);receipt.composition=composition;receipt.checks.push('The new-project expression defaults produced validated acoustic and recurrence sidecars, active vocal/semantic planning, legal FSEQ frames and an identical restored frame hash.');
