...
(async()=>{
 ...
 const receipt={release:'2.3.0',passed:false,checks:[],errors:[],source_hashes:{},scope:'Desktop Chromium with actual WebGL and web workers; simulated Android bridge. No Android device or Tesla.'};
 ...
 try{
  ...
  assert.equal(crypto.createHash('sha256').update(bytes.subarray(offset)).digest('hex'),frameHash);assert.equal(exp.metadata.projectState.provenance.app,'2.3.0');assert.equal(exp.metadata.validation.synchronization.manual[0].status,'matched');
