...
OUT=ROOT/'qa/release-2.3.0'
HISTORICAL_OUT=ROOT/'qa/release-2.2.4'
WORK=Path(os.environ.get('LIGHTFORGE_MDX_COMPARISON_DIR',ROOT.parent/'native-mdx-comparison-2.3.0'))
...
def main():
    gate=OUT/'native-mdx-comparison-verification.json'
    receipt={'release':'2.3.0','passed':False,'errors':[],'source_hashes':{}}
    write_atomic(gate,receipt)
    try:
        if EVIDENCE_SESSION is not None and not re.fullmatch(r'[0-9a-f]{32,128}\Z',EVIDENCE_SESSION):
