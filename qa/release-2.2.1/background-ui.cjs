...
const inputs=['qa/release-2.2.1/background-ui.cjs','version.json','web/version.js','web/app.js','web/index.html','web/styles.css','web/cockpit.css','web/cockpit.js'];
const receipt={release:'2.2.1',passed:false,checks:[],errors:[],source_hashes:Object.fromEntries(inputs.map(p=>[p,crypto.createHash('sha256').update(fs.readFileSync(path.join(root,p))).digest('hex')])),scope:'Real Chromium UI with a simulated Android job/status bridge; native background lifetime is tested separately on Android.'};
(async()=>{
 ...
 try{
  ...
  for(const width of [320,393,768]){
   ...
   await page.addInitScript(()=>{
    ...
    window.qaBackground={job:{id:'qa-job',projectId:project.id,name:project.name,state:'running',progress:.37,stage:'Studio · passage 3 of 24 · 47%',analysisStage:'separation',createdAt:Date.now()-245000,updatedAt:Date.now(),passageIndex:3,passageCount:24,passagesCompleted:2,restoredPassages:1,resumeAvailable:true},saves:0,continued:false,settings:[]};
    window.Android={pickAudio(){},getBootstrap:()=>JSON.stringify({projects:[project],lastProjectId:project.id,version:'2.2.1',backgroundJob:qaBackground.job,deviceCapabilities:capabilities}),
   ...
   receipt.checks.push('Elapsed time, passage completion/reuse, delayed progress, cancellation recovery and device settings fit '+width+'px; background/power controls dispatch correctly and active inputs remain frozen.');await page.close();
  }
