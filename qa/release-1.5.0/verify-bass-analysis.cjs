...
(async()=>{
 ...
 const sources=['web/analysis/bass-notes.js','web/analysis/dsp.js','web/analysis/wav-reader.js','tests/bass-notes.test.cjs','qa/release-1.5.0/verify-bass-analysis.cjs'],real=[];
 for(const [name,file]of [['Sample','sample.wav'],['Glass Castle available prefix','glass-castle.wav']]){
  const full=path.join(__dirname,'fixtures',file);if(!fs.existsSync(full))continue;
