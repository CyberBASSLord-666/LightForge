...
const hashes=()=>Object.fromEntries(['web/cockpit.js','web/cockpit.css','web/version.js','web/app.js','web/index.html','web/styles.css','web/precision-studio.js','web/engine/worker.js','web/engine/client.js','web/engine/show-engine.js','web/engine/light-planner.js','web/engine/music-cues.js','web/engine/sync-review.js'].map(f=>[f,crypto.createHash('sha256').update(fs.readFileSync(path.join(root,f))).digest('hex')]));
(async()=>{
 const receipt={release:'2.1.0',passed:false,checks:[],errors:[],source_hashes:hashes(),scope:'Desktop Chromium with actual WebGL and web workers; simulated Android bridge. No Android device or Tesla.'};
 ...
 try{
  ...
  assert.equal(crypto.createHash('sha256').update(bytes.subarray(offset)).digest('hex'),frameHash);assert.equal(exp.metadata.projectState.provenance.app,'2.1.0');assert.equal(exp.metadata.validation.synchronization.manual[0].status,'matched');
  ...
  for(const width of [320,393,768,1440]){
   await page.setViewportSize({width,height:960});await page.locator('#workspace-compose').click();await page.locator('#carCanvas').scrollIntoViewIfNeeded();await page.evaluate(async()=>{await LightForgeApp.vehiclePreview.ready;LightForgeApp.renderFrame(0);await new Promise(requestAnimationFrame);await new Promise(requestAnimationFrame);});await page.screenshot({path:path.join(out,'precision-'+width+'.png'),fullPage:true});
  ...
  for(const width of [393,1440]){await page.setViewportSize({width,height:960});for(const view of ['shows','guide']){await page.locator('.nav-item[data-navigate="'+view+'"]').click();assert.equal(await page.locator('#'+view).isVisible(),true);assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=document.documentElement.clientWidth+1));await page.screenshot({path:path.join(out,view+'-'+width+'.png'),fullPage:true});}}
  const fresh=await browser.newPage({viewport:{width:393,height:852}});fresh.on('pageerror',e=>errors.push(e.message));await fresh.addInitScript(()=>{window.Android={pickAudio(){},getBootstrap:()=>JSON.stringify({projects:[],version:'2.1.0'})};});await fresh.goto('http://127.0.0.1:'+server.address().port);await fresh.waitForFunction(()=>!!window.LightForgeApp);assert.equal(await fresh.locator('#emptyCard').isVisible(),true);assert.ok(await fresh.evaluate(()=>document.documentElement.scrollWidth<=document.documentElement.clientWidth+1));await fresh.screenshot({path:path.join(out,'first-launch-393.png'),fullPage:true});await fresh.close();receipt.checks.push('First launch, saved-show collection and vehicle guide render without overflow or page errors at phone and desktop sizes.');
