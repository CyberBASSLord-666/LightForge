...
const mask=new Float32Array(manifest.indices.length*1301*2);for(let i=0;i<mask.length;i+=2)mask[i]=1;
class Tensor{constructor(type,data,dims){this.data=data;this.dims=dims;}dispose(){}}
const ort={Tensor,InferenceSession:{create:async url=>({run:async({input})=>({output:new Tensor('float32',String(url).includes('head-')?mask:input.data,[])}),release:async()=>{}})}};
...
(async()=>{
 ...
 const sources=['web/analysis/separator-deux.js','web/analysis/dsp.js','web/analysis/models/deux/manifest.json','qa/release-2.1.0/test-source-clock.cjs'];const receipt={release:'2.1.0',passed:true,source_hashes:Object.fromEntries(sources.map(p=>[p,sha(path.join(root,p))])),scope:'Neutral masks and fake graph sessions; actual production STFT/scatter/ISTFT and overlapping chunk scheduler. No neural inference.',samples:total,chunks:result.chunks,maxAbsError:maximum,contiguousSourceSamples:true,monotonicProgress:true};fs.writeFileSync(path.join(__dirname,'source-clock-verification.json'),JSON.stringify(receipt,null,2));console.log(receipt);
