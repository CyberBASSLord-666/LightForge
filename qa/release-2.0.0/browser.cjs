...
(async()=>{
 ...
 try{
  ...
  for(const width of [320,393,768,1440]){
   await page.setViewportSize({width,height:960});await page.locator('#carCanvas').scrollIntoViewIfNeeded();await page.evaluate(async()=>{await LightForgeApp.vehiclePreview.ready;LightForgeApp.renderFrame(0);await new Promise(requestAnimationFrame);await new Promise(requestAnimationFrame);});await page.screenshot({path:path.join(out,'precision-'+width+'.png'),fullPage:true});
