...
async function analyze(audioUrl,options={},onProgress=()=>{},signal){
 ...
 const runOptions={...serializableOptions,cacheKey,workId,androidApp,supportsNativeDeux:hasNative,supportsNativeMdx:hasMdx,supportsGameCapacity:hasGameCapacity},timings={},started=performance.now();let value={},progress=0;
 function runStage(stage){return new Promise((resolve,reject)=>{
  ...
  worker.onmessage=e=>{
   ...
   if(m.type==='result'){if(m.runtime)scope.LightForgeDiagnostics?.log('info','analysis-runtime',JSON.stringify({stage,...m.runtime}));timings[stage]={seconds:Number.isFinite(m.seconds)?m.seconds:0,restored:!!m.restored};end(null,m.value);}
