...
const root=path.resolve(__dirname,'../..'),work=path.resolve(process.argv[2]||__dirname);
require(root+'/web/analysis/dsp.js');
const A=require(__dirname+'/separator-mdx-2.2.4.js'),B=require(root+'/web/analysis/separator-mdx.js'),Reader=require(root+'/web/analysis/wav-reader.js');
...
(async()=>{
 ...
 const report={passed:true,scope:'Exact real/stress-input frontend comparison. Local alternating warm CPU timings; excludes inference and bridge and is not a phone benchmark.',sources:{baseline:sha(fs.readFileSync(__dirname+'/separator-mdx-2.2.4.js')),candidate:sha(fs.readFileSync(root+'/web/analysis/separator-mdx.js')),dsp:sha(fs.readFileSync(root+'/web/analysis/dsp.js')),probe:sha(fs.readFileSync(__filename))},cases:reports,timings,medianMilliseconds:{baseline:median(timings.baseline),candidate:median(timings.candidate)},speedup:median(timings.baseline)/median(timings.candidate)};
 fs.writeFileSync(work+'/mdx-fft-verification.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({passed:true,cases:reports.length,medianMilliseconds:report.medianMilliseconds,speedup:report.speedup}));
