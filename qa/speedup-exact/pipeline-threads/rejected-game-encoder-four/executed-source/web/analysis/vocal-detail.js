...
function summarize(data,{classifier={},model,transcription}={}){
 ...
 // A singing-specific neural note is independent semantic evidence. It may
 // support an uncertain source phrase only where measured periodic pitch
 // agrees, the source is prominent, and speech does not dominate. Scores from
 // Frame-MN10 stay untouched; this is never a fabricated classifier score.
 const neuralPitch=new Float32Array(n),neuralSupport=new Uint8Array(n);
 for(const note of transcription?.notes||[]){
  if(!Number.isFinite(note.start)||!Number.isFinite(note.end)||!Number.isFinite(note.midi)||note.end-note.start<.12||note.midi<0||note.midi>127)continue;
  for(let i=Math.max(0,Math.ceil(note.start/STEP));i<Math.min(n,Math.ceil(note.end/STEP));i++)neuralPitch[i]=note.midi;
 }
 ...
 for(let i=0;i<n;i++){const x=i/block-.5,b=Math.floor(x),fraction=clamp(x-b),scale=scales[Math.max(0,b)]*(1-fraction)+scales[Math.min(blocks-1,b+1)]*fraction,e=maxRange(fineEnergy,i*4,i*4+4);singing[i]=classifierAt(classifier,i*STEP,'singing');speech[i]=classifierAt(classifier,i*STEP,'speech');const learned=Math.max(singing[i],speech[i]),quietEvidence=sourceRatio[i]>.08||learned>.4;reference[i]=Math.max(.00004,quietEvidence?Math.min(scale,localPeak[i]*1.15):scale);levels[i]=clamp(e/reference[i]);sourceSupport[i]=sourceRatio[i]>=(learned>.4?.0008:.008)?1:0;peakScore=Math.max(peakScore,singing[i]);const conf=periodicity[i]||0;pitch[i]=frequency[i]>0&&conf>=.45?69+12*Math.log2(frequency[i]/440):0;
  ...
  neuralSupport[i]=neuralPitch[i]>0&&pitch[i]>0&&Math.abs(neuralPitch[i]-pitch[i])<=1.5&&conf>=.55&&!(speech[i]>=.24&&speech[i]>singing[i]*1.35)?1:0;
  sourceLed[i]=sourceRatio[i]>=.025&&levels[i]>=.20&&(semanticContext[i]>=.035||neuralSupport[i])?1:0;
 ...
 for(const run of raw){let a=run.first,b=run.last+1;if((b-a)*STEP<.16-1e-8)continue;let maxSing=0,maxSpeech=0,voiced=0,peakLevel=0,peakFrame=a,totalEnergy=0,totalResidual=0,separatedSupport=0,contextScore=0,neuralFrames=0;
  for(let i=a;i<b;i++){separatedSupport+=sourceLed[i];neuralFrames+=neuralSupport[i]&&sourceLed[i]?1:0;contextScore=Math.max(contextScore,semanticContext[i]);maxSing=Math.max(maxSing,singing[i]);maxSpeech=Math.max(maxSpeech,speech[i]);if(pitch[i])voiced++;if(levels[i]>peakLevel){peakLevel=levels[i];peakFrame=i;}const e=fineEnergy[i*4]||0,r=residualEnergy?.[i*4]||0;totalEnergy+=e*e;totalResidual+=r*r;}
  ...
  const neuralAdmission=!speechDominates&&neuralFrames*STEP>=.20&&neuralFrames/(b-a)>=.35&&stemRatio>=.025;
  const sourceLedAdmission=separatedSupport*STEP>=.20&&separatedSupport/(b-a)>=.35&&stemRatio>=.025&&(contextScore>=.035||neuralAdmission);
  ...
  const phrase={start,end,releaseTime:end,confidence:round(confidence),modelScore:round(maxSing,4),speechScore:round(maxSpeech,4),strength:round(peakLevel),peakTime:round(clamp(peakFrame*STEP,start,end)),kind,evidenceMode:neuralAdmission&&maxSing<.11&&maxSpeech<.19?'neural-note-supported':sourceLedAdmission&&maxSing<.11&&maxSpeech<.19?'separation-led':'classifier-supported',estimated:true,source:'separated-vocals',voicedFraction:round(voicedFraction),stemEnergyRatio:round(stemRatio,4)};phrases.push(phrase);a=Math.max(0,Math.floor(start/STEP));b=Math.min(n,Math.ceil(end/STEP));
