...
async function analyze(audioUrl,options={},onProgress=()=>{},signal){
 ...
 const androidApp=typeof scope.Android?.pickAudio==='function'||typeof scope.BackgroundJob?.checkpoint==='function';
 ...
 const runOptions={...serializableOptions,cacheKey,workId,androidApp,supportsNativeDeux:hasNative,supportsNativeMdx:hasMdx,supportsGameCapacity:hasGameCapacity},timings={},started=performance.now();let value={},progress=0,voiceClassifierSeconds=0;
 function runStage(stage){return new Promise((resolve,reject)=>{
  ...
  worker.onmessage=e=>{
   ...
   if(m.type==='result'){
    if(m.runtime)scope.LightForgeDiagnostics?.log('info','analysis-runtime',JSON.stringify({stage,...m.runtime}));
    const seconds=Number.isFinite(m.seconds)?m.seconds:0;
    if(stage==='voice-classifier')voiceClassifierSeconds+=seconds;
    else timings[stage]={seconds:seconds+(stage==='voice'?voiceClassifierSeconds:0),restored:!!m.restored};
    end(null,m.value);
   }
 ...
 try{
  for(const stage of STAGES){
   try{
    // Frame-MN10 keeps its original Android arithmetic in a private worker.
    // Retire that entire heap before GAME starts its qualified thread pool.
    // Its checkpoint is the handoff; public stages and musical fields stay put.
    if(stage==='voice'&&androidApp)await runStage('voice-classifier');
    value=await runStage(stage);
   }
   catch(error){
