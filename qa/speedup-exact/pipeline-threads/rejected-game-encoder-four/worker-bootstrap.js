...
let probeStage=null,probeAndroidApp=false,sessionSequence=0,wrapped=false;
self.addEventListener('message',event=>{if(event.data?.stage){probeStage=event.data.stage;probeAndroidApp=event.data.options?.androidApp===true;}});
...
self.importScripts=(...urls)=>{
 ...
 if(!wrapped&&self.ort){
  ...
  ort.InferenceSession.create=async function(source,options){
   const expectedThreads=['separation','voice'].includes(probeStage)?4:1;if(!probeAndroidApp||ort.env.wasm.numThreads!==expectedThreads||!crossOriginIsolated||typeof SharedArrayBuffer!=='function')throw Error('Requested final Android thread policy was not active');
   const sessionId=++sessionSequence,modelPath=new URL(String(source),self.location.href).pathname;
   await sendRecord({type:'session',stage:probeStage,sessionId,modelPath,threads:ort.env.wasm.numThreads,androidApp:probeAndroidApp,hardwareConcurrency:navigator.hardwareConcurrency,crossOriginIsolated,sharedArrayBuffer:typeof SharedArrayBuffer==='function',options:JSON.parse(JSON.stringify(options))});
