...
self.onmessage=async e=>{
 if(e.data?.type==='game-capacity-result'){
  const pending=gameCapacityRequests.get(e.data.requestId);if(!pending)return;gameCapacityRequests.delete(e.data.requestId);
  if(e.data.error)pending.reject(new Error(e.data.error));else pending.resolve(e.data.parallelism===2?2:1);return;
 }
 ...
 const configuredThreads=wasmThreadsForStage(options,stage,quality);
 ...
 if(gameParallelism===2)await gameCapacity('release');
