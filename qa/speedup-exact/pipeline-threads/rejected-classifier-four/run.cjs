...
const receipt={schema:'lightforge-full-pipeline-android-stage-threads-1',passed:false,scope:'Candidate-only actual public MusicAnalyzer replay against the preserved 350-graph original one-thread baseline. Final production Android stage routing is exercised with a probe-only Android.pickAudio marker, actual COI/SAB and navigator.hardwareConcurrency=8. This is host byte qualification, not a physical Android provider test or controlled timing.',input,controls:{quality:'precision',baselineHardwareConcurrency:2,candidateHardwareConcurrency:8,threadPolicy:'Final Android production route: rhythm1, Precision separation4, voice4, bass1; actual COI/SAB and >=8 cores required.',expectedEffectiveThreads:{rhythm:1,separation:4,voice:4,bass:1},androidMarker:'window.Android.pickAudio is a probe-only function; no method is called.',nativePrediction:false,gamePool:false,checkpoints:'Fresh independent browser context per run; same fixed analysisIdentity equal to WAV SHA256.'},sourceHashes:{},baselineBinding:{path:baselinePath,sha256:sha(baselinePath),originalSourceHashes:baseline.sourceHashes,originalExecutedSourceSnapshot:path.resolve(OUT,'../initial-1-vs-4/executed-source')},runs:{'1':baseline.runs['1'],'4':{sessions:[],graphs:[]}},errors:[],firstMismatch:null};
