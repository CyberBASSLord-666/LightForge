...
const fixture=path.join(OUT,'falcon-3s.wav'),sha=p=>crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');
const input=JSON.parse(fs.readFileSync(path.join(OUT,'input-binding.json')));assert.equal(sha(fixture),input.fixtureSha256);
const receipt={schema:'lightforge-full-pipeline-wasm-threads-1',passed:false,scope:'Actual public MusicAnalyzer, unchanged production adapters and model files, same original float32 source prefix, fresh Chromium storage for each run. This is host 1-vs-4 WASM thread qualification, not Android device timing.',input,controls:{quality:'precision',baselineHardwareConcurrency:2,candidateHardwareConcurrency:8,threadPolicy:'Unchanged production worker computes min(4,max(1,floor(hardwareConcurrency/2))) with crossOriginIsolated and SharedArrayBuffer required.',nativePrediction:false,gamePool:false,checkpoints:'Fresh independent browser context per run; same fixed analysisIdentity equal to WAV SHA256.'},sourceHashes:{},runs:{'1':{sessions:[],graphs:[]},'4':{sessions:[],graphs:[]}},errors:[],firstMismatch:null};
...
function comparable(record){const {type,stage,sessionId,call,modelPath,inputs,outputs}=record;return canonical({type,stage,sessionId,call,modelPath,inputs,outputs});}
...
(async()=>{
 ...
 try{
  ...
  for(const threads of [1,4]){
   ...
   await context.addInitScript(({threads})=>{const OriginalWorker=window.Worker;window.Worker=class extends OriginalWorker{constructor(url,options){const target=new URL(String(url),location.href);if(target.pathname==='/analysis/worker.js')super(new URL('/analysis/thread-probe-worker.js?run='+threads,location.href),options);else super(url,options);}};},{threads});
