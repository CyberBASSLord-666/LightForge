...
| Studio native CPU uses eight threads when at least eight cores are available | `native-threads/verification.json`: two real full-context passages, each with identical 4,586,400-byte output | Actual production classes, pinned native ORT 1.25.1 on Linux x86_64. Measured 16.8% and 20.8% less passage time. One to seven cores retain the old policy. MDX is unchanged. |
...
| GAME processes two independent passages using the existing worker and one child | Actual Android production-adapter qualification is required before release | Both lanes use the same single-thread WASM engine. Admission requires a fresh memory snapshot, a 64-bit process, at least four cores, at least 7 GiB total RAM and 4 GiB available RAM, and no low-memory signal. Results become durable independently and are stitched in source order. Failure disables pooling for that audio/app version and retries retained PCM serially. |
