#!/usr/bin/env python3
"""Bind fresh 2.3.1 source-clock and analysis evidence to a historical anchor.
...
inventory, and a fresh source-clock regression for v2.3.1. It does not claim a
...
def verify_historical_comparison(root, hashes):
    ...
    Current 2.3.1 source bytes are bound by fresh session receipts, not by this
...
def verify_profile_equivalence(root, hashes, comparison, evidence_session=EVIDENCE_SESSION_UNSET):
    # The paired proof is fresh v2.3.1 evidence; v2.2.4 is only recorded context.
...
def verify_clock(root, hashes, evidence_session=None):
    ...
    require(result.get('release') == RELEASE and result.get('passed') is True and not result.get('errors'),
            'Fresh 2.3.1 source-clock check did not pass')
...
def verify_release(root=ROOT):
    ...
    receipt = {
        ...
        'checks': [
            ...
            'Fresh 2.3.1 source-clock execution preserves all 932143 samples across four overlapping windows, including the final odd sample.',
