...
const inputs=['qa/release-2.2.3/background-ui.cjs','version.json','web/version.js','web/app.js','web/diagnostics.js','web/diagnostics.css','web/index.html','web/styles.css','web/cockpit.css','web/cockpit.js'];
const receipt={release:'2.2.3',passed:false,checks:[],errors:[],source_hashes:Object.fromEntries(inputs.map(p=>[p,crypto.createHash('sha256').update(fs.readFileSync(path.join(root,p))).digest('hex')])),scope:'Real Chromium UI with a simulated Android job/status bridge; native background lifetime is tested separately on Android.'};
(async()=>{
 ...
 try{
  ...
  for(const width of [320,393,768]){
   ...
   await page.addInitScript(()=>{
    ...
    window.Android={pickAudio(){},logDiagnostic:(...args)=>qaBackground.logs.push(args),exportDiagnostics(){qaBackground.exports++;},getBootstrap:()=>JSON.stringify({projects:[project],lastProjectId:project.id,version:'2.2.3',backgroundJob:qaBackground.job,deviceCapabilities:capabilities}),
