...
const sources=['web/analysis/separator-deux.js','web/analysis/dsp.js','web/analysis/models/deux/manifest.json','qa/release-2.2.3/test-source-clock.cjs'];
...
(async()=>{
 ...
 assert.deepEqual(hashes(),sourceHashes,'Source changed while verifying its clock');const receipt={release:'2.2.3',passed:true,errors:[],source_hashes:sourceHashes,scope:'Neutral masks and fake graph sessions; actual production STFT/scatter/ISTFT and overlapping chunk scheduler. No neural inference.',samples:total,chunks:result.chunks,maxAbsError:maximum,contiguousSourceSamples:true,monotonicProgress:true};fs.writeFileSync(path.join(__dirname,'source-clock-verification.json'),JSON.stringify(receipt,null,2));console.log(receipt);
