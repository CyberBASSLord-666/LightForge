...
const root = path.resolve(__dirname, '../..'), port = 8806, origin = `http://127.0.0.1:${port}`;
const chrome = path.resolve(root, '../toolchain/chrome/chrome-headless-shell-linux64/chrome-headless-shell');
const tracked = ['web/studio-tools.js', 'web/channel-studio.js', 'web/music-insights.js', 'web/index.html', 'web/styles.css', 'web/app.js', 'web/engine/client.js', 'web/engine/worker.js', 'web/engine/show-engine.js'];
...
(async () => {
  ...
  try {
    ...
    receipt.sourceStable=JSON.stringify(firstHashes)===JSON.stringify(hashes()); receipt.sourceHashes=hashes();receipt.passed=true;
