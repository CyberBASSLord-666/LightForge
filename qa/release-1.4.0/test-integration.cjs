/* Production browser/neural/project/export integration for LightForge 1.4.0. */
...
const receipt={release:'1.4.0',started:new Date().toISOString(),checks:[],errors:[],externalRequests:[],limits:['Desktop Chromium WebGL2/SwiftShader, not a physical phone or Tesla test.','Native export validator executes on the host JVM with production Java sources; Android codec/device execution is not claimed.']};
const startHashes=hashes();
async function snapshot(page){return page.evaluate(async()=>{const a=LightForgeApp,s=a.state;const digest=async bytes=>Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',bytes))).map(x=>x.toString(16).padStart(2,'0')).join('');const light=[];for(let f=0;f<s.show.frameCount;f++)for(let c=0;c<30;c++)light.push(s.show.frames[f*200+c]);return{projectId:s.project.id,settings:JSON.stringify(s.settings),cues:s.settings.manualCues,mutes:s.settings.outputEnabled,frames:await digest(s.show.frames),lights:await digest(new Uint8Array(light)),fseq:await digest(ShowEngine.fseq(s.show)),audio:await digest(await(await fetch(s.project.audioUrl)).arrayBuffer()),music:JSON.stringify(s.music),analysisVersion:s.music.analysisVersion||1,workers:window.__workers.filter(u=>u.includes('/analysis/worker.js')),history:s.history.length,validation:s.show.validation,targets:s.show.choreography.targets};});}
async function ready(page){await page.waitForFunction(()=>window.LightForgeApp?.vehiclePreview?.getSnapshot()?.modelReady&&!LightForgeApp.state.loadingProject&&!LightForgeApp.state.composing,{},{timeout:60000});}
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.evaluate(()=>LightForgeApp.state.music.analysisVersion),3);
  assert.equal(await page.evaluate(()=>LightForgeApp.state.music.engine.neural),true);
  assert.equal(await page.evaluate(()=>ShowEngine.version),'1.4.0');
  ...
  receipt.checks.push('Actual file-picker import converts the bundled 64-second music to project audio; a real offline Beat This! transformer ONNX worker returns analysisVersion 3 and generates a valid 1.4.0 show.');
  ...
  const legacy=JSON.parse(fs.readFileSync(path.join(root,'qa/music-1.3.0/baseline-demo-music.json'),'utf8'));
  await page.evaluate(async music=>{const a=LightForgeApp;a.state.music=music;await a.regenerate();await a.saveProject();},legacy);
  ...
  await page.locator('#channelStudio > summary').click();await page.evaluate(()=>ChannelStudio.select('left-combined'));await page.locator('.cue-open').first().click();await page.locator('#cueStart').fill('2.5');await page.locator('#saveCue').click();await page.waitForFunction(()=>!LightForgeApp.state.composing);
  ...
  await page.locator('#reanalyzeMusic').click();await page.waitForFunction(()=>!LightForgeApp.state.busy&&LightForgeApp.state.music?.analysisVersion===3,{},{timeout:180000});
  ...
  receipt.checks.push('A stored version-1 analysis recalls and edits without a neural rerun. Re-analyze upgrades to version 3 while preserving project identity, exact audio, edited light/RGB/mirror cues and muted outputs.');
  await page.locator('#recomposeShow').click();await page.waitForFunction(()=>!LightForgeApp.state.composing);const varied=await snapshot(page);assert.notEqual(varied.frames,upgraded.frames);assert.notEqual(varied.lights,upgraded.lights);assert.equal(varied.music,upgraded.music);assert.equal(varied.workers.length,upgraded.workers.length);assert.deepEqual(varied.cues,upgraded.cues);assert.deepEqual(varied.mutes,upgraded.mutes);
  ...
  receipt.checks.push('New variation changes actual light-frame bytes without another analysis worker or audio analysis; custom cues and mutes persist. Undo restores the exact previous FSEQ and settings.');
  ...
  const reloaded=await snapshot(page);assert(await page.evaluate(()=>!!LightForgeApp.state.compiled?.sha256));assert.equal(reloaded.fseq,final.fseq);assert.equal(reloaded.settings,final.settings);assert.equal(reloaded.audio,final.audio);assert.equal(reloaded.projectId,final.projectId);assert.equal(reloaded.workers.length,0);
  ...
  assert.equal(exported.project.compiled.sha256,final.frames);
  ...
  const officialPath=path.join(root,'qa/hardware-1.2.0/fixtures/official-integration-1.4.fseq');fs.writeFileSync(officialPath,regenerated);const official=cp.execFileSync('python3',[path.join(root,'research/hardware-1.2.0/commands-official-validator.py'),officialPath],{encoding:'utf8',input:'\n'});assert(!/ERROR|INVALID/.test(official));receipt.officialValidator=official;
  ...
  const javaRoot=path.resolve(root,'../toolchain/jdk17/bin'),androidJar=path.resolve(root,'../toolchain/android-sdk/platforms/android-35/android.jar'),jsonJar=path.resolve(root,'../toolchain/test-json.jar'),classes=path.join(out,'native-integration-classes');fs.mkdirSync(classes,{recursive:true});
  ...
  const sourceList=[...javaSources,path.join(root,'build/generated/com/cyberbasslord/lightforge/R.java'),path.join(root,'tests/NativeHardwareTest.java'),path.join(root,'qa/music-1.3.0/NativeMusicIntegrationTest.java')];
