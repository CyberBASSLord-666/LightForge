...
Current source is the **2.2.4 release candidate**. Three Android attempts remain failed or incomplete: an initial teardown ANR, a reopened-preview readiness timeout, and a focused diagnostic run that also timed out waiting for JavaScript. The latest startup fix prevents duplicate completed-project restoration and defers preview GPU work until the model is ready and visible. Current local regression passes all 347 Node and 69 Python tests, including 22 focused startup checks; the tested completed Studio/Night renders are pixel-identical. Fresh complete Android verification and a new original-signed APK are pending. [2.2.2](https://github.com/CyberBASSLord-666/LightForge/releases/tag/v2.2.2) remains the latest published update.
