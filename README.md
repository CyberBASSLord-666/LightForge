...
**LightForge 2.2.5 / Android version code 20205 is a release candidate, not a published update.** It awaits a fresh source-bound verification run, protected performance-quality approval, and original-signed APK validation. Physical phone and Tesla observations are optional for publication and remain unverified when absent. [LightForge 2.2.4](https://github.com/CyberBASSLord-666/LightForge/releases/tag/v2.2.4) remains the published update; install that original-signed APK over an existing app to preserve private projects.
