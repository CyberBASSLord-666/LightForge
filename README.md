...
Current source is the **2.2.4 release candidate**. The verification/build job and original-signed build pass, and focused Android diagnostics pass eight checks. However, [the corrected-fixture Android run](qa/release-2.2.4/android-corrected-fixture-run/README.md) still times out reopening its completed project. The portable fixture proof confirms a real settings mismatch in the earlier test, but correcting it has not resolved the remaining Android failure. Publication remains blocked on complete lifecycle and final package gates. [2.2.2](https://github.com/CyberBASSLord-666/LightForge/releases/tag/v2.2.2) remains the latest published update.
